...
def seed_categories():
    ...
    with app.app_context():
        ...
        categoria_saidas = Categoria(
            nome="Saídas", tipo=SAIDA, descricao="Todas as despesas"
        )
        categoria_entradas = Categoria(
            nome="Entradas", tipo=ENTRADA, descricao="Todas as receitas"
        )
        ...
        print(
            f"Pais criados: Saídas (ID: {categoria_saidas.id}) e Entradas (ID: {categoria_entradas.id})"
        )
        ...
        subcategorias = [
            ...
            Categoria(
                nome="Energia Elétrica", tipo=SAIDA, parent_id=categoria_saidas.id
            ),
            Categoria(nome="Agua", tipo=SAIDA, parent_id=categoria_saidas.id),
            Categoria(
                nome="Internet/Streaming", tipo=SAIDA, parent_id=categoria_saidas.id
            ),
            Categoria(nome="Supermercado", tipo=SAIDA, parent_id=categoria_saidas.id),
            Categoria(
                nome="Restaurante/Lanche", tipo=SAIDA, parent_id=categoria_saidas.id
            ),
            Categoria(
                nome="Transporte Público/Uber",
                tipo=SAIDA,
                parent_id=categoria_saidas.id,
            ),
            Categoria(nome="Combustível", tipo=SAIDA, parent_id=categoria_saidas.id),
            Categoria(
                nome="Manutenção Veicular", tipo=SAIDA, parent_id=categoria_saidas.id
            ),
            Categoria(
            ...
            Categoria(
                nome="Educação/Cursos", tipo=SAIDA, parent_id=categoria_saidas.id
            ),
            Categoria(nome="Lazer/viagens", tipo=SAIDA, parent_id=categoria_saidas.id),
            Categoria(
                nome="Manutenção Residencial", tipo=SAIDA, parent_id=categoria_saidas.id
            ),
            ...
            Categoria(
                ...
                tipo=ENTRADA,
                parent_id=categoria_entradas.id,
            ),
            Categoria(
                nome="Freelance/Extra", tipo=ENTRADA, parent_id=categoria_entradas.id
            ),
        ]
